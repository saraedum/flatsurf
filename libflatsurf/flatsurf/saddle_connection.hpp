...
class SaddleConnection : public Serializable<SaddleConnection<Surface>>,
 ...
 public:
  ...
  // with `side`.
  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> inHalfPlane(const Surface &, HalfEdge side, const Vertical<Surface> &, const Vector<T> &);
  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> inPlane(const Surface &, HalfEdge plane, const Vector<T> &);
  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> inPlane(const Surface &, HalfEdge plane, const Vertical<Surface> &direction);
  [[deprecated("Use SaddleConnection() instead and determine source & target with surface.sector()")]] static SaddleConnection<Surface> inPlane(const Surface &, HalfEdge sourcePlane, HalfEdge targetPlane, const Chain<Surface> &);

  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> alongVertical(const Surface &, const Vertical<Surface> &direction, HalfEdge plane);

  // Return the saddle connection strictly clockwise from the given saddle
  // connection with the given direction.
  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> clockwise(const SaddleConnection &from, const Vector<T> &);
  ...
  [[deprecated("Use SaddleConnection() instead and determine the source & target with surface.sector()")]] static SaddleConnection<Surface> counterclockwise(const Surface &, HalfEdge source, HalfEdge target, const Chain<Surface> &);

  // Return the saddle connection that starts counterclockwise (or collinear)
  // from source and goes in `direction`.
  [[deprecated("Use inSector() instead and determine the sector with surface.sector()")]] static SaddleConnection<Surface> counterclockwise(const Surface &, const SaddleConnection<Surface> &source, const Vertical<Surface> &direction);
